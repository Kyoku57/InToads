...
+(NSMutableArray *)searchObjectsForEntity:(NSString*)entityName withPredicate:(NSPredicate *)predicate andSortKey:(NSString*)sortKey andSortAscending:(BOOL)sortAscending fromOffset:(int)offset andNbOfRecords:(int)maxRecords andContext:(NSManagedObjectContext *)managedObjectContext
{
    ...
    if (maxRecords>-1) {
        [request setFetchLimit:maxRecords];
    }
...
// search with predicate without record limitation
+(NSMutableArray *)searchObjectsForEntity:(NSString*)entityName withPredicate:(NSPredicate *)predicate andSortKey:(NSString*)sortKey andSortAscending:(BOOL)sortAscending andContext:(NSManagedObjectContext *)managedObjectContext
{
    return [self searchObjectsForEntity:entityName withPredicate:predicate andSortKey:sortKey andSortAscending:sortAscending fromOffset:-1 andNbOfRecords:-1 andContext:managedObjectContext];
}
...
+(NSMutableArray *)searchObjectsForEntity:(NSString*)entityName withPredicate:(NSPredicate *)predicate andContext:(NSManagedObjectContext *)managedObjectContext {
    return [CoreDataHelper searchObjectsForEntity:entityName withPredicate:predicate andSortKey:nil andSortAscending:false andContext:managedObjectContext];
}

// search for unique object, or return nil if not found
+(id)searchObjectForEntity:(NSString*)entityName withPredicate:(NSPredicate *)predicate andContext:(NSManagedObjectContext *)managedObjectContext {
    NSMutableArray *results = [self searchObjectsForEntity:entityName withPredicate:predicate andSortKey:nil andSortAscending:true fromOffset:-1 andNbOfRecords:1 andContext:managedObjectContext];
...
+(void) truncateAll:(NSPersistentStoreCoordinator *)storeCoordinator; {
    ...
    for (NSPersistentStore* s in [storeCoordinator persistentStores]) {
        NSURL *storeURL = s.URL;
        [storeCoordinator removePersistentStore:s error:&error];
